        ...
        if idx >= 0 and time_allowance > 0 and budget > 0:
            ...
            if place.cost <= budget and place.time_needed <= time_allowance:
                ...
                if prev_res.score + place.score >= prev_res_same_budget.score:
                    new_result = memo[time_allowance, budget, idx]
...
if __name__ == "__main__":
    ...
    res = multiDimensionalKnapsack(4, 2, [place_a, place_b, place_c])
    pp(res.score)  # prints 8.5
    pp([place.name for place in res.places])  # prints ['B', 'C']
